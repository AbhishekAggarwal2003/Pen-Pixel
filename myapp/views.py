...
def create(request):
    if request.method == 'POST':
        ...
        image = request.FILES.get('image')  # This will be None if no image is uploaded

        if not postname or not content or not category:
            messages.error(request, "Post name, content, and category are required.")
